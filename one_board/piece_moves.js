...
class Pawn extends Piece{
    ...
    promote(square, move){
        is_promoting = true
        if (this.side == "white") {
            promote_div.innerHTML = `<div width: 100px; height: 320px style="border-radius: 10px;position: relative; width: 100px; height: 320px; background-color: crimson;">
...
function draw_board(moved_square) {
    ...
    for (let h = 0; h < 2; h++) {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            for (let j = 0; j < piece.length; j++) {
                ...
                if (i == 0){
                    var square = document.getElementById("piece_slot_" + piece[j].notation_square)
        ...
        var square = document.getElementById("piece_slot_" + player.king.notation_square)
        square.innerHTML = `<img src="${piece_img}.png" alt="piece" height="70px">`
    }
    ...
    for (let i = 0; i < 8; i++) {
        for (let j = 0; j < 8; j++) {
            ...
            var square = document.getElementById(files[j] + ranks[7 - i])
            square.style = `background-color: ${color};`
        }
    }

    if (white.king.is_checked == true) {
        var square = document.getElementById(white.king.notation_square)
        square.style = `background-color: rgb(253, 52, 52);`
    }
    if (black.king.is_checked == true) {
        var square = document.getElementById(black.king.notation_square)
        square.style = `background-color: rgb(253, 52, 52);`
    }

    if (moved_square) {
        var square = document.getElementById(moved_square)
...
function clear_board(notation_square) {
    var square = document.getElementById("piece_slot_" + notation_square)
    square.innerHTML = ``
}
